	...
	: _pWin(new uch[2 * WSIZE])
{
	for (int i = 0; i < 2; i++)
	{
		_LiteralLengthBuf[i].reserve(BUFF_SIZE);
		_DistBuf[i].reserve(BUFF_SIZE);
		_FlagBuf[i].reserve(BUFF_SIZE / 8);
	}
...
{
	e.start();
	if (syscall(__NR_sched_setaffinity, syscall(SYS_gettid), sizeof(cpu_set_t), cpu_set[0]) != 0)
	{
		std::cout << "Error in setting affinity for thread " << sched_getcpu() << std::endl;
	}
	/* LZ77压缩思想 */
	// 1、以二进制方式打开文件
	...
	while (lookAhead != 0)
	{
		...
		//auto starttime1 = getTimeNs();
		// b-1.更新哈希表
		matchHead = 0;													// 每次循环，插入新的三字符组，需重置匹配链头
		_ht.InsertString(hashAddr, _pWin[start + 2], start, matchHead); // 更新matchHead，即当前三字符组在_prev的匹配链头
		//auto endtime1 = getTimeNs();
		//auto updatehashduration = (endtime1 - starttime1);
		//updateHash+=updatehashduration;
		//cout << "lz77(update_HashTable): " << updatehashduration.count() << " milliseconds" << endl;
		// b-2.查找最长匹配
		//auto starttime2 = getTimeNs();
		if (matchHead) // 有匹配，需查找最长匹配
		{
			matchLength = LongestMatch(matchHead, start, matchDist, lookAhead);
		}
		//auto endtime2 = getTimeNs();
		//auto LongestMatchduration = (endtime2 - starttime2);
		//longestmatch+=LongestMatchduration;
		//cout << "lz77(LongestMatch): " << LongestMatchduration.count() << " milliseconds" << endl;
		
		auto starttime3 = getTimeNs();
		// b-3.根据匹配情况添加缓冲区
		...
		auto endtime3 = getTimeNs();
		auto saveMatchduration = (endtime3 - starttime3);
		saveMatch+=saveMatchduration;
		//cout << "lz77(saveMatch): " << saveMatchduration.count() << " milliseconds" << endl;
		// b-4.刷新窗口
		// cout << !feof(fIn) << endl;
		if (!feof(fIn))
		{
	...
	if (bitCount > 0 && bitCount < 8) // bitInfo最后可能不够8比特位导致数据残留
	{
		_isLast = true;
		bitInfo <<= (8 - bitCount);
		_FlagBuf[lz77Buffer].push_back(bitInfo);
		HfCompress();
	}

	if (_isLast)
	{
		//auto lz77end = high_resolution_clock::now();
		...
		//auto lz77duration = duration_cast<milliseconds>(lz77end - begin);
		auto duration = duration_cast<milliseconds>(end - begin);
		//cout << "lz77(update_HashTable): " << updateHash<< " ns" << endl;
		//cout << "lz77(LongestMatch): " << longestmatch<< " ns" << endl;
		//cout << "lz77(saveMatch): " << saveMatch<< " ns" << endl;
		//cout << "lz77: " << lz77duration.count() << " milliseconds" << endl;
		e.end();
        e.out("/data/local/tmp/energySamplerResult-cpu-bs.csv");
		std::ofstream out("/data/local/tmp/result-cpu-bs.csv");
		out<<"type,time"<<endl;
		out<<"all"<<","<<duration.count()<<endl;
...
void MyGZip::SaveLZ77Info(uch matchLength, ush matchDist, uch &bitInfo, uch &bitCount, ull lookAhead)
{
	...
	_LiteralLengthBuf[lz77Buffer].push_back(matchLength); // 传入字符/长度缓冲区
	...
	{
		bitInfo |= 1;
		_DistBuf[lz77Buffer].push_back(matchDist);
	}
	...
	if (BUFF_SIZE == _LiteralLengthBuf[lz77Buffer].size())
	{
...
{
	changeBuffer.lock();
	lz77Buffer = 1 - lz77Buffer;
	threads.push_back(std::thread([&]
								  {
									  if (syscall(__NR_sched_setaffinity, syscall(SYS_gettid), sizeof(cpu_set_t), cpu_set[1]) != 0)
									  {
										  std::cout << "Error in setting affinity for thread " << sched_getcpu() << std::endl;
									  }
									  auto begin = high_resolution_clock::now();
									  ...
									  auto end = high_resolution_clock::now();
									  auto duration = duration_cast<milliseconds>(end - begin);
									  //printf("Huffman: %ld milliseconds\n", duration.count());
									  changeBuffer.unlock();
									  // cout << "Huffman: " << duration.count() << " milliseconds" << endl;
								  }));
}

// huffman压缩接口
void MyGZip::_HfCompress()
{
	// 0、清空上次的Info信息数组
	ClearInfo();

	// 1、统计源文件中每个字节的出现次数
	int f = StatAppearCount();
	...
	for (size_t i = 0; i < _LiteralLengthBuf[1 - lz77Buffer].size(); ++i)
	{
		if (bitCount == 0)
		{
			bitInfo = _FlagBuf[1 - lz77Buffer][index_FlagBuf++];
			bitCount = 8;
		}
		if (bitInfo & 0x80)
		{
			CompressLength_Dist(_LiteralLengthBuf[1 - lz77Buffer][i], _DistBuf[1 - lz77Buffer][index_DistBuf++], compressBitInfo, compressBitCount);
		}
		else
		{
			CompressLiteral(_LiteralLengthBuf[1 - lz77Buffer][i], compressBitInfo, compressBitCount);
		}
		bitInfo <<= 1;
		bitCount--;
	}
	...
	_LiteralLengthBuf[1 - lz77Buffer].clear();
	_DistBuf[1 - lz77Buffer].clear();
	_FlagBuf[1 - lz77Buffer].clear();
}
...
int MyGZip::StatAppearCount()
{
	...
	for (size_t i = 0; i < _LiteralLengthBuf[1 - lz77Buffer].size(); ++i)
	{
		...
		if (bitInfo & 0x80)
		{
			_LiteralLengthInfo[GetIndexLengthCode(_LiteralLengthBuf[1 - lz77Buffer][i])]._appearCount++;
			_DistInfo[GetIndedDistCode(_DistBuf[1 - lz77Buffer][index_DistBuf++])]._appearCount++;
			discount++;
		}
		else
		{
			_LiteralLengthInfo[_LiteralLengthBuf[1 - lz77Buffer][i]]._appearCount++;
		}
		bitInfo <<= 1;
		bitCount--;
	}
